...
{
    int N = std::pow(2,10);
    float time_instance, my_mv_time=0.f, smem_mv_time=0.f, cublas_mv_time=0.f;
    double my_mv_bandwidth, smem_mv_bandwidth, cublas_mv_bandwidth;
    ...
    for(int i = 0; i < 10; i++){
        hipEventRecord(start);
        cublasSgemv(handle, op, N, N, &alpha, d_A.elements, N, d_x1, 1, &beta, d_y, 1);
        hipEventRecord(stop);
        hipEventSynchronize(stop);
        hipEventElapsedTime(&time_instance, start, stop);
        if( i > 0) cublas_mv_time += time_instance; 
    }
    cublas_mv_time /= 9;
    ...
    cublas_mv_bandwidth = 4*numer/(cublas_mv_time*1e6);
    ...
    std::cout<< "cublasSgemv time = " << cublas_mv_time << " ms" << std::endl;
    std::cout<< "cublasSgemv Bandwidth = " << cublas_mv_bandwidth << " GB/s" << std::endl;
}
