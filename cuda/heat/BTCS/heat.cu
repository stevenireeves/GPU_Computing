...
__global__ void tridiag_gj(float *diag, float *subdiag, float *supdiag,
                           float *x, float *xout, float *b) {
...
void tridiag_par_gj(float *diag, float *sub, float *sup, float *x, float *b,
                    float eps) {
  float res = 1.0f;
  float *xnew;
  float *dres;
...
void BTCS(float *f, float dt, float dx, float kappa, float tend, int tio) {
  ...
  create_tridiag<<<dimGrid, dimBlock>>>(diag, sub, sup,
                                        r); // creates tridiag for BTCS
  while (t < tend) {
