...
__global__ void smem_histo(int *d_bins, const int *d_in, const int BIN_COUNT,
                           const int size) {
...
int main() {
  ...
  // Launch Kernel
  //	faux_histo<<<ARRAY_SIZE/BIN_COUNT, BIN_COUNT>>>(d_bins, d_in,
  //BIN_COUNT); 	simple_histo<<<ARRAY_SIZE/BIN_COUNT, BIN_COUNT>>>(d_bins, d_in,
  //BIN_COUNT);
  smem_histo<<<ARRAY_SIZE / (32 * BIN_COUNT), BIN_COUNT, BIN_BYTES>>>(
      d_bins, d_in, BIN_COUNT, ARRAY_SIZE);
  cudaEventRecord(gpuStop, 0);
  cudaEventSynchronize(gpuStop);
  cudaEventElapsedTime(&gpuElapsedTime, gpuStart,
                       gpuStop); // time in milliseconds
